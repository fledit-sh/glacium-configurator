...
def list_docs(conn, serial: str):
    cur = conn.execute(
        "SELECT doc_type, doc_version, filename, uploaded_at "
        "FROM docs WHERE serial=? ORDER BY uploaded_at DESC",
        (serial,),
    )
    return cur.fetchall()
...
def save_doc(
    ...
    safe_name = (
        f"{serial}__{doc_type}__v{doc_version}__{ts}__{original_name}".replace("/", "_")
    )
    out_path = FILES_DIR / safe_name
    out_path.write_bytes(file_bytes)

    conn.execute(
        "INSERT INTO docs(serial, doc_type, doc_version, filename, uploaded_at) "
        "VALUES(?,?,?,?,?)",
...
def device_page(conn, serial: str):
    ...
    if not docs:
        ...
        for doc_type, doc_version, filename, uploaded_at in docs:
            st.markdown(
                f"**{doc_type}** — Version: `{doc_version}` — Uploaded: `{uploaded_at}`"
            )
    ...
    if is_admin_logged_in():
        ...
        doc_type = st.text_input("Document type (e.g., DoC, Manual, TestReport)")
        doc_version = st.text_input("Version (e.g., 1.0.2)")
...
def main():
    ...
    # "Routing": Read serial number from query param: ?sn=KM1-000123
    q = st.query_params
    serial = q.get("sn", None)

    st.sidebar.subheader("Navigation")
    sn_input = st.sidebar.text_input("Open serial number", value=serial or "")
    if st.sidebar.button("Open") and sn_input:
